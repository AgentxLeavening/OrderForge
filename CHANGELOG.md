...
## Unreleased
- **Sell a finished good directly** on an order without a template: the new-order modal can pick an inventory item, set a quantity + packing time, and get the same pricing breakdown, suggested price/margin, and stock deduction (modeled as a synthetic one-line template). Ideal for pre-made items that just need packaging.
- Orders now **persist the price breakdown** (material_cost, labor_cost, markup, fee_pct — migration 013) captured at creation from a template. The order detail page shows an internal **Pricing & Margin** card (materials, labor, cost, markup, fee, suggested price, and estimated profit + margin %). Margin is intentionally internal-only — not shown on the customer invoice.
